...
This library is a successor to all the elm-community/\*-extra packages, as well as several other community packages, such as GlobalWebIndex/cmd-extra, hayleigh-dot-dev/tuple-extra, stoeffel/set-extra, munksgaard/char-extra, matthewsj/elm-ordering (albeit somewhat lossely that one).
...
elm-json uninstall elm-community/array-extra elm-community/basics-extra elm-community/dict-extra elm-community/list-extra elm-community/maybe-extra elm-community/result-extra elm-community/string-extra GlobalWebIndex/cmd-extra hayleigh-dot-dev/tuple-extra stoeffel/set-extra munksgaard/char-extra
...
If you are improving existing functions please demonstrate performance by adding both the old and new versions of the functions into the `/benchmarks` directory.

If you are proposing breaking changes to existing functionality, please add an automatic migration to the `/upgrade` script so that version upgrades can be automated and please include detailed rationale for the changes being proposed.
